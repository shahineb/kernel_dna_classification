...
cpdef float _evaluate(str seq1,
    ...
    cdef np.ndarray[np.float64_t, ndim=2] M = np.zeros((min_len, max_len))
    cdef np.ndarray[np.float64_t, ndim=2] X = np.zeros((min_len, max_len))
    cdef np.ndarray[np.float64_t, ndim=2] Y = np.zeros((min_len, max_len))
    cdef np.ndarray[np.float64_t, ndim=2] X2 = np.zeros((min_len, max_len))
    cdef np.ndarray[np.float64_t, ndim=2] Y2 = np.zeros((min_len, max_len))
